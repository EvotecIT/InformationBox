...
public static class FixRegistry
{
    private static readonly FixAction[] BuiltInFixes =
    {
        ...
        new()
        {
            ...
            Command = "$exe = @(\"$env:LOCALAPPDATA\\Microsoft\\OneDrive\\OneDrive.exe\", \"C:\\Program Files\\Microsoft OneDrive\\OneDrive.exe\") | Where-Object { Test-Path $_ } | Select-Object -First 1; if ($exe) { Stop-Process -Name OneDrive -ErrorAction SilentlyContinue; Start-Sleep -Seconds 2; Start-Process $exe; Write-Output 'OneDrive restarted.' }",
        ...
        new()
        {
            ...
            Category = FixCategory.OneDrive,
            Command = "$exe = @(\"$env:LOCALAPPDATA\\Microsoft\\OneDrive\\OneDrive.exe\", \"C:\\Program Files\\Microsoft OneDrive\\OneDrive.exe\") | Where-Object { Test-Path $_ } | Select-Object -First 1; if ($exe) { Start-Process $exe -ArgumentList '/reset' -Wait; Start-Sleep -Seconds 5; Start-Process $exe; Write-Output 'OneDrive reset complete.' }",
